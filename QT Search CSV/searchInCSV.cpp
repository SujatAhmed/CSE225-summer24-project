...
void searchInCSV(const QString &csvFileName, const std::string &dataFolder) {
    ...
    if (!table) {
        QMessageBox::critical(nullptr, "Error", "Table not found for file: " + csvFileName);
        return;
    }

    // Display a search dialog
    bool ok;
    QString searchKey = QInputDialog::getText(nullptr, "Search in " + csvFileName, "Enter code to search:", QLineEdit::Normal, QString(), &ok);
    ...
    if (ok && !searchKey.isEmpty()) {
        ...
        if (result) {
            ...
            QObject::connect(closeButton, &QPushButton::clicked, &resultWindow, &QWidget::close);
            ...
            QMessageBox::warning(nullptr, "Not Found", "Code not found in the table.");
        }
    } else {
        QMessageBox::information(nullptr, "Search Canceled", "Search canceled or empty input.");
    }
}
