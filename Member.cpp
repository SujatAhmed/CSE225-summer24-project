...
void MemberList::saveToCSV() {
    ...
    Member* temp = head;
    while (temp) {
        file << temp->name << "," << temp->username << ","
             << temp->password << "," << temp->memberType << "\n";
        temp = temp->next;
    }
    file.close();
}
...
bool MemberList::registerMember(string name, string username, string password, string type) {
...
string MemberList::loginMember(string username, string password) {
    ...
    while (temp) {
        if (temp->username == username && temp->password == password) {
            cout << "Login successful! Welcome, " << temp->name << " (" << temp->memberType << ").\n";
